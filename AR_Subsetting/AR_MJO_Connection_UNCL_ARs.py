...
#This script by Chad Small & Brandon Kerns checks to see if AR Split/Merge Systems that had unclear connections connect to the MJO LPT at any time during thier lifetime
...
pac_ars_df = pd.read_csv('/home/disk/orca/csmall3/AR_testing_research/Final_AR_Results/text_files/Pac_Basin_MJO_Connected_AR_Split_Merge_UNCL.csv')
...
for jj in range(0,len(pac_ars_df)):
    ...
    for j in range(0, len(ar_test['time'])):
        ...
        if len(mjo_slice) == 0:
            ...
            AR_ID += [str(pac_ars_df['AR ID (string)'].iloc[jj])]
            ...
            AR_ivt += [pac_ars_df['Max IVT'].iloc[jj]]
            AR_ID_num += [str(pac_ars_df['AR ID (Number)'].iloc[jj])]
            AR_year_end += [pac_ars_df['End Season Year'].iloc[jj]]
            ...
            if bool_match == True:
                ...
                # fig.show()
                plt.savefig('/home/disk/orca/csmall3/AR_testing_research/Final_AR_Results/Figures/Pac_Bas_AR_MJO_Match_Unclear_ARs/'+lpt_id_tit+'_'+year_str+'.png')
...
AR_MJO_overlp_df.to_csv('/home/disk/orca/csmall3/AR_testing_research/Final_AR_Results/text_files/Pac_Basin_MJO_Connected_AR_Split_Merge_DEF_pt2.csv') 