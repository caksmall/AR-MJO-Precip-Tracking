...
def get_points_from_lpo(lpo_id):
    ...
    data_dir_lpo = (
        '/home/orca/bkerns/projects/mjo_lpt_and_ar/tracking/'
        + 'lpt-python-public/ar.testing7.merge_split/data/ar/g0_0h/thresh1/objects'
    )
...
output_file = '/home/disk/orca/csmall3/AR_testing_research/Final_AR_Results/text_files/Total_Global_Splt_MRG_ARs_'+str(instance)+'.csv'
...
if __name__ == '__main__':
    ...
    year_2 = 1 + year1
    test_ls = sorted(glob.glob('/home/orca/bkerns/projects/mjo_lpt_and_ar/tracking/lpt-python-public/ar.testing7.merge_split/data/ar/g0_0h/thresh1/systems/'+str(year1)+'060100_'+str(year_2)+'063023/*.nc'))


    # I have it doing the first 10 for testing. You can have it do all of them.
    # for lptid in lptid_list[0:10]:
    for idx, lptid in enumerate(lptid_list, start=0):
    # for lptid in lptid_list:
        ...
        AR_PAC_df = pd.DataFrame({'AR ID (Number)': [str(lptid)], 
                                ...
                                'AR ID (string)':[str(test_ls[int(idx)])]})
